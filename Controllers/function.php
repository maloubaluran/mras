...
function get_tags($return_type = '') {
  ...
  if ($return_type != '' && is_array($tags) && $tags != '') {
    ...
    if ($return_type == 'styles') {
      ...
      foreach ($tags as $tag) {
        $output .= '<li><a href="#" class="title" id="tag_'.$title_counter.'">'.$tag['title'].' (<a href="../Controllers/function.php?delete=true&amp;id='.$tag['id'].'">Delete</a>)</a></li>';
        $title_counter++;
      }
    }
	}
...
if($_POST['tag']) {
	...
	if($errflag) {
		...
		session_write_close();
		header("location: ../Views/Assessment.php");
		exit();
	}
