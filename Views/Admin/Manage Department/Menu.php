<div class="art-nav">
	<div class="l"></div>
	<div class="r"></div>

	<ul class="art-menu">								
     	<li>
           <a href="../Home.php"><span class="l"></span><span class="r"></span><span class="t">Home</span></a>
        </li>
        <li>
            <a href="#"><span class="l"></span><span class="r"></span><span class="t">Manage User's Account</span></a>
			<ul style="width:180px;">
				<li>
					<a href="../Manage User Account/Add User Account.php"><span class="l"></span><span class="r"></span><span class="t">Register A User</span></a>
				</li>
				<li>
					<a href="../Manage User Account/History User Account.php"><span class="l"></span><span class="r"></span><span class="t">User Accounts</span></a>
				</li>
			</ul>
		</li>
		<li>
            <a href="#"><span class="l"></span><span class="r"></span><span class="t">Manage Department</span></a>
			<ul style="width:160px;">
				<li>
					<a href="Add Department.php"><span class="l"></span><span class="r"></span><span class="t">Add Department</span></a>
				</li>
				<li>
					<a href="History Department.php"><span class="l"></span><span class="r"></span><span class="t">Department List</span></a>
				</li>
			</ul>
		</li>
		<li>
            <a href="#"><span class="l"></span><span class="r"></span><span class="t">Manage Record</span></a>
			<ul style="width:150px;">
				<li>
					<a href="../Manage Record/Upload Test Result.php"><span class="l"></span><span class="r"></span><span class="t">Upload Test Result</span></a>
				</li>
				<li>
					<a href="../Manage Record/View Test Result.php"><span class="l"></span><span class="r"></span><span class="t">View Test Result</span></a>
				</li>
			</ul>
		</li>
        <li>
              <a href="About Us.php"><span class="l"></span><span class="r"></span><span class="t">About Us</span></a>
         </li>
         <li>
              <a href="../../../Controllers/Controller.php?logout"><span class="l"></span><span class="r"></span><span class="t">Log Out</span></a>
        </li>
	</ul>
</div>
